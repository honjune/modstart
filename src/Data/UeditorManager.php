...
class UeditorManager
{
    ...
    private static function saveToUser($uploadTable, $user_id, $data)
    {
        ModelUtil::insert($uploadTable, [
            'category' => $data['category'],
            'data_id' => $data['id'],
            'upload_category_id' => 0,
            'user_id' => $user_id,
        ]);
    }
    ...
    public static function handle($uploadTable, $uploadCategoryTable, $user_id, $option = null)
    {
        ...
        switch ($action) {
            ...
            case 'image':
                DataUploadingEvent::fire($uploadTable, $user_id, 'image');
                $editorRet = [
                    'state' => 'SUCCESS',
                    'url' => null
                ];
                ...
                self::saveToUser($uploadTable, $user_id, $ret['data']['data']);
                $editorRet['url'] = $ret['data']['fullPath'];
                DataUploadedEvent::fire($uploadTable, $user_id, 'image', $ret['data']['data']['id']);
