        ...
        $form = new Form(AdminRole::class, function (Form $form) {
            ...
            $form->hookSaving(function (Form $form) {
                if (FormMode::EDIT == $form->mode()) {
                    ...
                    if (!empty($deletes)) {
                        AdminRoleRule::where(['role_id' => $form->itemId()])->whereIn('rule', $deletes)->delete();
                    }
                    $user_ids = $form->item()->users->pluck('id')->toArray();
                    if (!empty($user_ids)) {
                        AdminUser::whereIn('id', $user_ids)->update(['rule_changed' => true]);
                    }
                }
            });
