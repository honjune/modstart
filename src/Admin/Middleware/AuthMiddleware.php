...
class AuthMiddleware
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        $admin_user_id = intval(Session::get('_admin_user_id', null));
        $adminUser = null;
        ...
        if (empty($admin_user_id)) {
            ...
            if ($authAdminUserId > 0) {
                ...
                if (empty($authAdminUser['password']) || empty($authAdminUser['password_salt'])) {
                    return Response::json(-1, "admin user forbidden");
                }
                $md5String = "$authAdminTimestamp:$authAdminRequestId:$authAdminUserId:$authAdminUser[username]:$authAdminUser[password]:$authAdminUser[password_salt]";
                $signCalc = md5($md5String);
                if ($signCalc != $authAdminSign) {
                    return Response::json(-1, 'admin user sign error');
                }
                $admin_user_id = $authAdminUser['id'];
        ...
        if (!$this->isAuthIgnore($urlController, $urlMethod)) {
            ...
            if (AdminPermission::isFounder($admin_user_id)) {
                foreach ($rules as $k => $v) {
                    $rules[$k]['auth'] = true;
                }
            } else {
                $adminHasRules = Session::get('_adminHasRules', []);
                if (true || (empty($adminHasRules) && $adminUser['id'] > 0) || $adminUser['rule_changed']) {
                    if ($adminUser['rule_changed']) {
                        Admin::rule_changed($adminUser['id'], false);
                    }
                    $adminHasRules = [];
        ...
        Session::put('_admin_user_id', $admin_user_id);
